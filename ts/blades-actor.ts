	...
	updateRandomizers() {
		...
		const randomGen: Record<string, (gender?: string) => string | string[] | false> = {
			...
			trait: () => sampleArray(Randomizers.trait, [
				(<BladesActorOfType<BladesActorType.npc>>this).system.persona.trait1.value,
				(<BladesActorOfType<BladesActorType.npc>>this).system.persona.trait2.value,
				(<BladesActorOfType<BladesActorType.npc>>this).system.persona.trait3.value
			], 1),
		...
		const gender = (<BladesActorOfType<BladesActorType.npc>>this).system.persona.gender.isLocked
			? (<BladesActorOfType<BladesActorType.npc>>this).system.persona.gender.value
			: randomGen.gender() as "M"|"F"|"U"|"X";
		const updateKeys = (<Array<KeyOf<BladesActorSchema.NPC["persona"]>>>Object.keys((<BladesActorOfType<BladesActorType.npc>>this).system.persona))
			.filter((key) => !(<BladesActorOfType<BladesActorType.npc>>this).system.persona[key]?.isLocked);
			...
			switch (key) {
				case "gender": {
					updateData[`system.persona.${key}`] = {
						isLocked: (<BladesActorOfType<BladesActorType.npc>>this).system.persona.gender.isLocked,
						value: gender
					};
					break;
				}
				...
				default: {
					...
					updateData[`system.persona.${key}`] = {
						isLocked: false,
						value: randomVal || (<BladesActorOfType<BladesActorType.npc>>this).system.persona[key].value
					};
					break;
				}
			}

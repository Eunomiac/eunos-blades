...
class BladesActor extends Actor implements BladesDocument<Actor> {
  ...
  static get All() { return game.actors as Collection<BladesActorOfType<"ANY">> }
  static Get(actorRef: ActorRef): BladesActorOfType<"ANY"> | undefined {
    if (actorRef instanceof BladesActor) { return actorRef as BladesActorOfType<"ANY"> }
    if (U.isDocID(actorRef)) { return BladesActor.All.get(actorRef as string) }
  ...
  static GetTypeWithTags<T extends BladesActorType>(docType: T, ...tags: BladesTag[]): Array<BladesActorOfType<T>> {
    return BladesActor.All
      ...
      .filter((actor): actor is BladesActorOfType<T> => actor.hasTag(...tags));
  }
  ...
  get tags(): BladesTag[] { return this.system.tags ?? [] as BladesTag[] }

import {BladesActorType, BladesItemType} from "./constants";
...
const AI = {
  QuerySystem: (keyword: string) => {
    // Scans in-game objects for keyword. Crafts system message that incorporates retrieved data.
    if (keyword) { return keyword } else { return "This is a placeholder." }
  },
  QueryAssistant: (type: BladesActorType|BladesItemType, keys: string[]) => {
    // Given a type and a list of keys, will return suitable examples of JSON objects containing those keys.
    if (type && keys) { return [type, keys] } else { return "This is a placeholder." }
  }
};

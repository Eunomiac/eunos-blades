...
class BladesActorSheet extends ActorSheet {
  ...
  async _onToggleValueClick(event: ClickEvent) {
    event.preventDefault();
  ...
  async _validateOrRevealSubData(downtimeAction: DowntimeAction, actionSubData?: string): Promise<boolean> {
    switch (downtimeAction) {
      case DowntimeAction.LongTermProject: {
        ...
        if (BladesProject.IsType(projectItem)) { return true; }
      ...
      case DowntimeAction.Recover: {
        ...
        if (healerActor instanceof BladesActor && healerActor.hasTag(Tag.NPC.CanHeal)) { return true; }
  ...
  async _onDowntimeActionClick(event: ClickEvent) {
    ...
    const downtimeAction = elem$.data("action").substring(elem$.data("action").lastIndexOf("-") + 1) as DowntimeAction;
    ...
    switch (downtimeAction) {
      ...
      case DowntimeAction.Train: {
        const [attr, value] = actionSubData.split(/:/) as [AttributeTrait|"playbook", number];
        if (attr === "playbook") {

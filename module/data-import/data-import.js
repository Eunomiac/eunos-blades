...
export const UpdateClaims = async () => {
    ...
    claim.forEach((cl) => {
        ...
        const [turfNum] = Object.entries(playbookObj.system.turfs ?? {}).find(([tNum, tData]) => tData.name === cl.name) ?? [];
        if (!turfNum) {
            errorReport.push(`Claim ${cl.name} has invalid turf name ${cl.name} for playbook ${cl.playbook}`);
            return;
        }
        playbookUpdateData[playbookObj.name] ??= {};
        playbookUpdateData[playbookObj.name][`system.turfs.${turfNum}.description`] = cl.rules;
        if (cl.flavor) {
            playbookUpdateData[playbookObj.name][`system.turfs.${turfNum}.flavor`] = cl.flavor;
        }
    });
...
export const UpdateOps = async () => {
    ...
    favoredOperation.forEach((op) => {
        ...
        }).then((item) => item?.addTag(playbookObj.name));
    });
};
